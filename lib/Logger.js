...
var Logger = {
  init : function(options) {
    ...
    this.dataMerge = options.dataMerge || false;
  ...
  send : function(event, msg) {
    ...
    if (typeof msg === 'object') {
      if (msg instanceof Error)
        this.sock.emit(event, msg.stack + "");
      else
	this.sock.emit(event, JSON.stringify(msg));
    }
    else {
  ...
  buildLogger : function(event) {
    ...
    if (!event) throw new Error('You have have to define a prefix');
    return function(msg) {
